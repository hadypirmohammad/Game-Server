...
module.exports = class Server {
    ...
    CheckUserOnline(telId){
        let server = this;
        let cons = server.connections;
        let count = 0;

        for (let x in cons) {
            let player = cons[x].player;
            
            if(player.telid == telId){
                count = count + 1;
                if(count == 2){
                    console.log("same player in server left game");
                }
            }
        }
    ...
    onDisconnected(connection = Connection) {
        ...
        if(connection.lobby.peopleInLobby > 1){
            connection.lobby.peopleInLobby = connection.lobby.peopleInLobby - 1;
            connection.lobby.lose(connection.player);
        }
        
        
        console.log(connection.lobby.peopleInLobby);
    ...
    onAttemptToJoinGame(connection = Connection) {
        ...
        if(!lobbyFound) {
            ...
            let gamelobby = new GameLobby(new GameLobbySettings('FFA', 2, 1, levelData1));
